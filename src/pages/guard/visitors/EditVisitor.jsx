...
const EditVisitor = () => {
  ...
  const fetchVisitor = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const formattedEntryTime = formatDateForInput(data.entryTime);
        const formattedExitTime = formatDateForInput(data.exitTime);
        ...
        setFormData({
          ...
          idType: data.idType || 'CNIC',
          idNumber: data.idNumber || '',
          notes: data.notes || '',
          status: data.status || 'Verified'
        });
      } else {
        toast.error(response.data.message || 'Failed to load visitor data');
        navigate('/guard/visitors');
      }
    } catch (error) {
      ...
      if (error.response?.status === 401) {
        ...
        toast.error('Invalid visitor ID format');
        navigate('/guard/visitors');
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`${API_URL}/guard/visitors/${id}`, formData, {
        headers: { 
          Authorization: `Bearer ${token}` 
        }
      });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-white">Edit Visitor</h1>
          <p className="text-text-muted text-sm mt-1">Update visitor information.</p>
        </div>
